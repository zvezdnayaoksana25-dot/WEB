(() => {
  ...
  const CATEGORIES = ['Greeting', 'Flirty', 'Engagement', 'Control / Direction style', 'Custom'];
  ...
  const load = (key, fallback) => safeJson(localStorage.getItem(key), fallback);
  ...
  function startSession() {
    const session = { id: uid(), start: new Date().toISOString() };
  ...
  function startTimerLoop() {
    ...
    const tick = () => {
      ...
      $('#sessionState').textContent = active ? `Started ${new Date(active.start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : 'Ready to begin a focused block.';
  ...
  function renderHistory(sessions) {
    $('#sessionHistory').innerHTML = sessions.slice(0, 8).map((s) => `
      <div class="history-row"><span><strong>${new Date(s.start).toLocaleDateString()}</strong><br>${new Date(s.start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span><span>${formatDuration(s.duration)}</span></div>
